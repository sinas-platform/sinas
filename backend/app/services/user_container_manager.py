...
class UserContainerManager:
        ...
        while True:
            try:
                ...
                idle_timeout = getattr(
                    settings, "function_container_idle_timeout", 3600
                )  # 1 hour default
    ...
    async def get_or_create_container(self, user_id: str, db: AsyncSession) -> Container:
        ...
        async with self.container_lock:
            ...
            self.user_containers[user_id] = {"container": container, "last_used": time.time()}
            return container
    ...
    async def _install_packages(self, container: Container, user_id: str, db: AsyncSession):
        ...
        try:
            ...
            approved_packages = {
                pkg.package_name: pkg.version for pkg in approved_result.scalars().all()
            }
            ...
            logger.info(
                f"Installing {len(packages_to_install)} packages in container for user {user_id}: {', '.join(packages_to_install)}"
            )
...
""",
            ...
            stdout, stderr = exec_result.output
            logger.debug(
                f"Function sync exec result: exit_code={exec_result.exit_code}, stdout={stdout}, stderr={stderr}"
            )
            ...
            if exec_result.exit_code == 0:
                if stdout:
                    try:
                        ...
                        logger.info(
                            f"Loaded {total_functions} functions across {len(functions_data)} namespaces into container for user {user_id}"
                        )
                    ...
                    total_functions = sum(len(funcs) for funcs in functions_data.values())
                    logger.info(
                        f"Function sync completed (no output captured), attempted to load {total_functions} functions"
                    )
            else:
                error_msg = stderr.decode() if stderr else "Unknown error"
    ...
    async def execute_function(
        ...
        try:
            ...
            print(
                f"⏱️  [TIMING] Starting container exec_run for {function_namespace}/{function_name}"
            )
            exec_result = await asyncio.to_thread(
                container.exec_run,
                cmd=[
