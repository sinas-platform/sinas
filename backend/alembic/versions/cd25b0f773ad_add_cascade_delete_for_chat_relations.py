...
def upgrade() -> None:
    ...
    op.drop_constraint('pending_tool_approvals_chat_id_fkey', 'pending_tool_approvals', type_='foreignkey')
    op.create_foreign_key('pending_tool_approvals_chat_id_fkey', 'pending_tool_approvals', 'chats', ['chat_id'], ['id'], ondelete='CASCADE')

    # Update executions.chat_id to SET NULL on delete
    op.drop_constraint('executions_chat_id_fkey', 'executions', type_='foreignkey')
    op.create_foreign_key('executions_chat_id_fkey', 'executions', 'chats', ['chat_id'], ['id'], ondelete='SET NULL')
...
def downgrade() -> None:
    ...
    op.drop_constraint('pending_tool_approvals_chat_id_fkey', 'pending_tool_approvals', type_='foreignkey')
    op.create_foreign_key('pending_tool_approvals_chat_id_fkey', 'pending_tool_approvals', 'chats', ['chat_id'], ['id'])
