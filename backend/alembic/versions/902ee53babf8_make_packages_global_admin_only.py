...
def upgrade() -> None:
    ...
    op.drop_constraint('installed_packages_installed_by_fkey', 'installed_packages', type_='foreignkey')

    # Recreate installed_by as nullable foreign key (for audit trail)
    op.alter_column('installed_packages', 'installed_by',
                    existing_type=sa.UUID(),
                    nullable=True)
    op.create_foreign_key('installed_packages_installed_by_fkey', 'installed_packages', 'users', ['installed_by'], ['id'])
...
def downgrade() -> None:
    ...
    op.create_foreign_key('installed_packages_user_id_fkey', 'installed_packages', 'users', ['user_id'], ['id'])
    ...
    op.alter_column('installed_packages', 'user_id',
                    existing_type=sa.UUID(),
                    nullable=False)

    # Recreate original unique constraint
    op.create_unique_constraint('uix_package_user_name', 'installed_packages', ['user_id', 'package_name'])