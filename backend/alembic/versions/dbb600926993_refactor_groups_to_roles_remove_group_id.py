...
def upgrade() -> None:
    ...
    op.drop_column("scheduled_jobs", "group_id")
    ...
    op.execute(
        "ALTER INDEX ix_group_permissions_permission_key RENAME TO ix_role_permissions_permission_key"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER INDEX ix_role_permissions_permission_key RENAME TO ix_group_permissions_permission_key"
    )
    ...
    op.add_column("scheduled_jobs", sa.Column("group_id", sa.UUID(), nullable=True))
    ...
    op.create_foreign_key(None, "scheduled_jobs", "groups", ["group_id"], ["id"])
    ...
    op.create_index("ix_scheduled_jobs_group_id", "scheduled_jobs", ["group_id"])
