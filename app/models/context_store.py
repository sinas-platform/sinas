"""Context store model for user/group/assistant context management."""
...
class ContextStore(Base):
    """Flexible key-value store for user context, preferences, reminders, and learned information."""
    ...
    __table_args__ = (
        ...
        Index(
            "uq_context_store_user_namespace_key",
            "user_id", "namespace", "key",
            unique=True
        ),
        # Performance indexes
        Index("ix_context_stores_namespace_visibility", "namespace", "visibility"),
        Index("ix_context_stores_expires_at", "expires_at"),
    )
